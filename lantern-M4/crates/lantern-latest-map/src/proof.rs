...
/// Returns the empty root of the exactly pinned JMT v0.12/SHA-256 profile.
///
/// M4 needs this pre-state value when the first block closes an empty epoch
/// before it creates latest-map version zero. The storage tests assert that an
/// actual empty version produces this exact root.
#[must_use]
pub const fn empty_latest_root() -> Hash32 {
    Hash32::new(*b"SPARSE_MERKLE_PLACEHOLDER_HASH__")
}
