...
fn strict_control_verifier_checks_key_id_and_signature() -> TestResult {
    ...
    if let ControlActionV1::Enable {
        initial_admin_key, ..
    } = &mut substituted.action
    {
        *initial_admin_key = Ed25519PublicKey::new([0x55; 32]);
    }
    assert_eq!(
...
fn every_v1_domain_label_is_frozen_and_separated() -> TestResult {
    let domains = [
        ...
        (DomainV1::HistoryRecord, "lantern/v1/history-record"),
        (DomainV1::CaState, "lantern/v1/ca-state"),
        (DomainV1::StateTransaction, "lantern/v1/state-transaction"),
        (
            DomainV1::TransactionResults,
            "lantern/v1/transaction-results",
        ),
        (DomainV1::EpochBundle, "lantern/v1/epoch-bundle"),
        (DomainV1::AdminRegistry, "lantern/v1/admin-registry"),
        (DomainV1::SchemaConfig, "lantern/v1/schema-config"),
