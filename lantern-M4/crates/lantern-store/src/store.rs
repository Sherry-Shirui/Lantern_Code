...
/// Reads the last typed commit metadata through any M1 read view.
///
/// This is the sole public accessor for the reserved metadata key and lets M4
/// perform recovery cross-checks without duplicating M1's on-disk encoding.
///
/// # Errors
///
/// Returns a backend error or rejects corrupt fixed-width metadata.
pub fn read_commit_metadata<S: ReadStore + ?Sized>(store: &S) -> Result<Option<CommitMetadataV1>> {
    store
        .get(ColumnFamily::Metadata, COMMIT_METADATA_KEY)?
