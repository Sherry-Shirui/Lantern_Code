...
last closed epoch/HeadID, validator configuration hash, and application config
hash. `current_metadata()` is therefore the single source for ABCI `Info` after
restart.
...
history size, last closed epoch/HeadID, validator/config hashes, and the sorted
