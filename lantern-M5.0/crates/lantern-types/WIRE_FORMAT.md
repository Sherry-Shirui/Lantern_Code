...
| Action | Array |
| --- | --- |
| Enable | `[1, initial_manifest_hash, initial_admin_key]`；32-byte Ed25519 key 被 control signature 覆盖 |
...
以下对象沿用第 1 节的 strict canonical-CBOR 规则：

- `HistoryRecordV1`：长度 11，顺序为
  `[version_tag, network, ca_id, ca_version, event_type, manifest_number?,
  manifest_hash?, previous_manifest_hash?, admission_epoch, flags,
  authorization_digest]`。event code 为 Publish=1、Enable=2、Disable=3、
  Cancel=4、Rollover=5、Terminal=6。
- `LatestValueV1`：长度 12；effective manifest 的 number/hash/intent digest
  必须同时存在或同时缺失。status code 为 Enabled=1、Disabled=2、
  Terminal=3；rollover link 只允许出现在 terminal rollover value。
- `CaStateV1`：长度 18；除 latest value 字段外还绑定 admin key/sequence、
  authenticated predecessor、expected first manifest 和 rollover link。
- `PublicationTransactionV1 = [intent, exact_manifest_der,
  intent_signature, ee_certificate_chain]`；manifest 最大 512 KiB，signature
  最大 8 KiB，chain 最多 16 项且每项最大 64 KiB。
- `ControlTransactionV1 = [event, authorization]`；
  `StateTransactionV1 = [1, publication]` 或 `[2, control]`。
- `TransactionResultV1` 长度 9，绑定原始 transaction ID、首次 height/index、
  stable result code 和 accepted history/version/epoch 三元组。accepted 三元组
  必须全有，rejected 必须全无。
- `StateConfigV1` 长度 8，绑定 network、genesis、30s integration 或 300s
  paper profile、validator/governance hashes、key epoch 和 catch-up 上限。

新增 hash domain：`HistoryRecord`、`CaState`、`StateTransaction`、
`TransactionResults`、`EpochBundle`、`AdminRegistry`、`SchemaConfig`。

## 8. Validator configuration 与 governance update
