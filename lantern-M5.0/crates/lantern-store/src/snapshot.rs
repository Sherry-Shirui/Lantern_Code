...
pub struct SnapshotManifestV1 {
    ...
    /// Last closed epoch; absent before epoch zero closes.
        ...
        match (&self.last_closed_head_id, self.last_closed_epoch) {
            ...
            (None, Some(_)) => {
                return Err(Error::Checkpoint(
                    "closed HeadID is absent for a present closed epoch".to_owned(),
                ));
            }
            (Some(_), None) => {
                return Err(Error::Checkpoint(
                    "closed HeadID is present without a closed epoch".to_owned(),
                ));
            }
        }
