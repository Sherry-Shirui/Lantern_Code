...
pub struct CommitMetadataV1 {
    ...
    /// Last epoch closed into a Lantern head; absent before epoch zero closes.
...
impl CommitMetadataV1 {
    ...
    /// # Errors
    ///
    /// Rejects a partially present closed-epoch/HeadID pair.
    pub fn validate(&self) -> Result<()> {
        if self.last_closed_epoch.is_some() != self.last_closed_head_id.is_some() {
            return Err(Error::Metadata(
                "closed epoch and HeadID must either both exist or both be absent".to_owned(),
            ));
        }
        Ok(())
    }
    ...
    pub(crate) fn encode(&self) -> Result<Vec<u8>> {
        ...
        bytes.extend_from_slice(&self.last_closed_epoch.unwrap_or(0).to_be_bytes());
    ...
    pub(crate) fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        let encoded_epoch = read_u64(bytes, 120)?;
        let head_flag = bytes[128];
        let head_bytes = read_hash(bytes, 129)?;
        let (last_closed_epoch, last_closed_head_id) = match head_flag {
            0 if encoded_epoch == 0 && head_bytes.as_bytes() == &[0; HASH_BYTES] => (None, None),
            0 if encoded_epoch != 0 => {
                return Err(Error::Metadata(
                    "absent closed head contains a non-zero epoch".to_owned(),
                ));
            }
            0 => {
                return Err(Error::Metadata(
            ...
            1 => (Some(encoded_epoch), Some(head_bytes)),
            other => {
                return Err(Error::Metadata(format!(
